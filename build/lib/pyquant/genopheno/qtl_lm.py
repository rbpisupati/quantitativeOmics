...
class QTLmapperLM(object):
    ...
    def get_qtl_maps(self, covs = None):
        ...
        if type(self.pheno) is pd.Series:
            lm = scan(self.genos[filter_nanaccs_ix, :], np.array( self.pheno.iloc[filter_nanaccs_ix] ), covs = covs[filter_nanaccs_ix] )
            ...
            for cl in self.pheno:
                lm.append(scan(self.genos[filter_nanaccs_ix, :], np.array( self.pheno[cl][filter_nanaccs_ix] ), covs = covs[filter_nanaccs_ix] ))
